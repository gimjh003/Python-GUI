...
def merge_image():
    try:
        ...
        image_sizes = [] # [(width1, height1), (width, height2), ...]
        if img_width > -1:
            # width 값 변경
        ...
        # [(10, 10), (20, 20), (30, 30)]
        widths, heights = zip(*(image_sizes)) # zip에 대해 공부해둘것
        ...
        # 포맷 옵션 처리
        file_name = "Myfirstphoto."+img_format
...
def start():
    ...
    # 파일 목록 획인
    if list_file.size() == 0: # 하나의 파일도 선택이 되지 않은 경우
...
txt_dest_path.pack(side="left", fill="x", expand=True, ipady=5, padx=5, pady=5) # 높이 변경
